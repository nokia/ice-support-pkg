...
from psutil._compat import int
...
class TestSystemAPITypes(PsutilTestCase):
    ...
    def test_cpu_freq(self):
        ...
        self.assert_ntuple_of_nums(psutil.cpu_freq(), type_=(float, int, int))

    def test_disk_io_counters(self):
        # Duplicate of test_system.py. Keep it anyway.
        for k, v in list(psutil.disk_io_counters(perdisk=True).items()):
            self.assertIsInstance(k, str)
            self.assert_ntuple_of_nums(v, type_=six.integer_types)
    ...
    def test_net_if_addrs(self):
        # Duplicate of test_system.py. Keep it anyway.
        for ifname, addrs in list(psutil.net_if_addrs().items()):
    ...
    def test_net_if_stats(self):
        # Duplicate of test_system.py. Keep it anyway.
        for ifname, info in list(psutil.net_if_stats().items()):
            self.assertIsInstance(ifname, str)
    ...
    def test_net_io_counters(self):
        # Duplicate of test_system.py. Keep it anyway.
        for ifname, _ in list(psutil.net_io_counters(pernic=True).items()):
            self.assertIsInstance(ifname, str)

    @unittest.skipIf(not HAS_SENSORS_FANS, "not supported")
    def test_sensors_fans(self):
        # Duplicate of test_system.py. Keep it anyway.
        for name, units in list(psutil.sensors_fans().items()):
    ...
    def test_sensors_temperatures(self):
        # Duplicate of test_system.py. Keep it anyway.
        for name, units in list(psutil.sensors_temperatures().items()):
            self.assertIsInstance(name, str)
            for unit in units:
                self.assertIsInstance(unit.label, str)
                self.assertIsInstance(unit.current, (float, int, type(None)))
...
class TestFetchAllProcesses(PsutilTestCase):
    ...
    def ppid(self, ret, info):
        self.assertIsInstance(ret, six.integer_types)
        self.assertGreaterEqual(ret, 0)
    ...
    def io_counters(self, ret, info):
        assert is_namedtuple(ret)
        for field in ret:
            self.assertIsInstance(field, six.integer_types)
            if field != -1:
