...
class Process(object):
    ...
    def _init(self, pid, _ignore_nsp=False):
        if pid is None:
            ...
            if not _PY3 and not isinstance(pid, six.integer_types):
    ...
    def __str__(self):
        ...
        with self.oneshot():
            ...
            return "%s.%s(%s)" % (
                ...
                ", ".join(["%s=%r" % (k, v) for k, v in list(info.items())]))
...
def disk_io_counters(perdisk=False, nowrap=True):
    ...
    if perdisk:
        ...
        return nt(*[sum(x) for x in zip(*list(rawdict.values()))])
...
if hasattr(_psplatform, "sensors_temperatures"):
    ...
    def sensors_temperatures(fahrenheit=False):
        ...
        for name, values in list(rawdict.items()):
            while values:
