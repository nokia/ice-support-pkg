...
from ._compat import int
from ._compat import lru_cache
from ._compat import range
from ._compat import six.text_type
...
def net_if_stats():
    ...
    for name, items in list(rawdict.items()):
        if not PY3:
...
def net_io_counters():
    ...
    return dict([(py2_strencode(k), v) for k, v in list(ret.items())])
...
class Process(object):
    ...
    def environ(self):
        ...
        if ustr and not PY3:
            assert isinstance(ustr, six.text_type), type(ustr)
    ...
    def cpu_affinity_set(self, value):
        ...
        for cpu in value:
            if cpu not in allcpus:
                if not isinstance(cpu, six.integer_types):
                    raise TypeError(
