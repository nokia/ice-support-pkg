...
def net_io_counters():
    ...
    for line in lines[2:]:
        ...
        (bytes_recv,
         ...
         carrierout,  # unused
         compressedout) = list(map(int, fields))
        ...
        for line in lines:
            ...
            if flen == 15:
                ...
                (reads_merged, rbytes, rtime, writes, writes_merged,
                    wbytes, wtime, _, busy_time, _) = list(map(int, fields[4:14]))
            elif flen == 14 or flen >= 18:
                # Linux 2.6+, line referring to a disk
                name = fields[2]
                (reads, reads_merged, rbytes, rtime, writes, writes_merged,
                    wbytes, wtime, _, busy_time, _) = list(map(int, fields[3:14]))
            elif flen == 7:
                # Linux 2.6+, line referring to a partition
                name = fields[2]
                reads, rbytes, writes, wbytes = list(map(int, fields[3:]))
    ...
    def read_sysfs():
        for block in os.listdir('/sys/block'):
            for root, _, files in os.walk(os.path.join('/sys/block', block)):
                ...
                (reads, reads_merged, rbytes, rtime, writes, writes_merged,
                    wbytes, wtime, _, busy_time) = list(map(int, fields[:10]))
